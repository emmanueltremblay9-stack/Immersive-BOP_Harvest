---
name: github-actions-secret-scope-audit
description: Use to audit actions secret scope in GitHub workflows with evidence-backed checks and bounded remediation.
---

# Github Actions Secret Scope Audit

## When to use

Use this skill when:

- A task explicitly mentions `github-actions-secret-scope-audit` behavior, github actions secret scope audit, or the same workflow intent.
- The request requires you to audit actions secret scope in GitHub workflows with evidence-backed checks and bounded remediation.
...
1. **Define the actions secret scope surface, expected behavior, and files or external targets in scope.**
...
- Github Actions Secret Scope Audit finding summary.
